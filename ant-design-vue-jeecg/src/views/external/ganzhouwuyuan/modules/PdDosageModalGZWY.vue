<template>
  ...
  >
    ...
    <pd-choose-dosage-list-model-g-z-w-y ref="pdChooseDosageListModel"  @ok="modalFormOk" ></pd-choose-dosage-list-model-g-z-w-y>
  </j-modal>
</template>
...
<script>
  ...
  import PdChooseDosageListModelGZWY from "./PdChooseDosageListModelGZWY";
  ...
  export default {
    name: "PdDosageModalGZWY",
    mixins: [JEditableTableMixin],
    components: {
      PdChooseProductStockListModel,
      PdChooseDosageListModelGZWY
    },
    data () {
      return {
    ...
    methods: {
      ...
      selectHis(num){ //查詢患者信息
        ...
        this.confirmLoading = true;
        let  formData={inHospitalNo:inHospitalNo,applicationNumber:applicationNumber};
          ...
          if (res.success) {
            if(res.result.length==1){
              res.result[0].patientDetailInfo="姓名:"+res.result[0].patientInfo+",性别:"+res.result[0].fsfXb+",住院号:"+res.result[0].inHospitalNo
                +",项目:"+res.result[0].operationName+",登记日期:"+res.result[0].oprDate;
      ...
      modalFormOk (formData) { //选择患者信息确定后返回所选择的数据
        formData.patientDetailInfo="姓名:"+formData.patientInfo+",性别:"+formData.fsfXb+",住院号:"+formData.inHospitalNo
          +",项目:"+formData.operationName+",登记日期:"+formData.oprDate;
