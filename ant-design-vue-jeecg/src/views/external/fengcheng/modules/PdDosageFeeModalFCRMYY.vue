<template>
  ...
  >
    ...
    <template slot="footer">
      ...
      <a-button @click="submitBtn" v-show="!disableSubmit" type="primary" :loading="confirmLoading" style="margin-right: 15px;">确认收费</a-button>
    </template>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          ...
          dosageCnclFee: "/pd/pdDosageFCRMYY/dosageCnclFee",//取消收费
          dosageFee: "/pd/pdDosageFCRMYY/dosageFee",//收费
          add: "/pd/pdDosage/add",
    ...
    methods: {
          ...
          if (res.success) {
            this.$nextTick(() => {
              ...
              if(this.model.id){
                ...
                this.hisPackageFlag = res.result.hisPackageFlagMax;
        ...
        getAction(this.url.queryPatientInfoList,formData).then((res)=>{
          if (res.success) {
            if(res.result.length==1){
              ...
              let fieldval = pick(res.result[0],'inHospitalNo','patientInfo','operativeNumber','operationName','outpatientNumber','medicalRecordNo','sqrtDoctorId','oprDeptId','oprDeptName','exeDeptId','exeDeptName','surgeonName','patientDetailInfo','hospitalizationsNum','remarks','extension1','extension2','subordinateWardName','visitNo','bedNumber','surgeonId','subordinateWardId');
              this.$nextTick(() => {
                this.form.setFieldsValue(fieldval);
              });
            }else{
        ...
        this.getAllTable().then(tables => {
          ...
          for (let i =0; i <list.length;i++){
            ...
            if(list[i].hyChargedText=='已退回'){
              this.$message.warning(list[i].productName+"产品已退回,无法进行收费");
              return;
            }else if(list[i].hyChargedText=='已收费'){
              this.$message.warning(list[i].productName+"产品已收费,无需再次收费");
              return;
            }else if(list[i].hyChargedText=='已退费'){
              this.$message.warning(list[i].productName+"产品已退费,无法再次收费");
              return;
            }
          }
      ...
      // 保存 提交 修改 请求函数
      request(formData) {
        let url = this.url.dosageFee, method = 'post'
        this.confirmLoading = true
        //是否收费标识
