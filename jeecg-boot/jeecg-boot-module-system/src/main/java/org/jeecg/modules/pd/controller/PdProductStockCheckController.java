...
import org.jeecg.modules.pd.vo.PdProductStockCheckChildPage;
...
public class PdProductStockCheckController {
     ...
     *
     * @param request
     * @param pdProductStockCheckChild
     */
    @RequestMapping(value = "/exportXls")
    public ModelAndView exportXls(HttpServletRequest request, PdProductStockCheckChild pdProductStockCheckChild) {
        // Step.1 组装查询条件查询数据
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();

        //Step.2 获取导出数据
        List<PdProductStockCheckChild> queryList = pdProductStockCheckChildService.pdProductStockCheckQueryList(pdProductStockCheckChild);
        List<PdProductStockCheckChildPage> exportList = JSON.parseArray(JSON.toJSONString(queryList), PdProductStockCheckChildPage.class);

        // Step.4 AutoPoi 导出Excel
        ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
        mv.addObject(NormalExcelConstants.FILE_NAME, "盘点记录表列表");
        mv.addObject(NormalExcelConstants.CLASS, PdProductStockCheckChildPage.class);
...
//		 Map<String, String> result = pdProductStockCheckService.audit(pdProductStockCheck, entity);
     ...
     * @return
     */
    @GetMapping(value = "/pdProductStockCheckQueryList")
    public Result<?> pdProductStockCheckQueryList(PdProductStockCheckChild pdProductStockCheckChild,
                                              @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                              @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
                                              HttpServletRequest req) {
        Page<PdProductStockCheckChild> page = new Page<PdProductStockCheckChild>(pageNo, pageSize);
        IPage<PdProductStockCheckChild> pageList = pdProductStockCheckChildService.pdProductStockCheckQueryList(page, pdProductStockCheckChild);
