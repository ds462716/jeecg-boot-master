 ...
 * @Description: 初始化库存
 * @Author: jiangxz
 * @Date: 2020年7月16日 10:44:10
 * @Version: V1.0
 */
@RestController
@RequestMapping("/pd/pdProductStockInit")
@Slf4j
public class PdProductStockInitController {
    @Autowired
    ...
    public Result<?> queryPageList(PdStockRecord pdStockRecord,
        ...
        pdStockRecord.setRecordType(PdConstant.RECODE_TYPE_3);
        IPage<PdStockRecord> pageList = pdStockRecordService.queryList(page, pdStockRecord, PdConstant.RECODE_TYPE_3);
    ...
    public Result<?> add(@RequestBody PdStockRecord pdStockRecord) {
        if(oConvertUtils.isEmpty(pdStockRecord.getId())){
            List<PdStockRecord> list = pdStockRecordService.queryList(pdStockRecord,PdConstant.RECODE_TYPE_3);
            if(CollectionUtils.isNotEmpty(list)){
                return Result.error("库存初始化单已被保存或提交，不能保存草稿！");
            }
        }else{
            ...
            if(entity != null && PdConstant.SUBMIT_STATE_2.equals(entity.getSubmitStatus())){
                return Result.error("库存初始化单已被提交，不能保存草稿！");
            }
        }

        String recordId = pdStockRecordService.saveMain(pdStockRecord, pdStockRecord.getPdStockRecordDetailList(), PdConstant.RECODE_TYPE_3);
    ...
    public Result<?> submit(@RequestBody PdStockRecord pdStockRecord) {
        if (oConvertUtils.isNotEmpty(pdStockRecord.getId())) {
            PdStockRecord entity = pdStockRecordService.getById(pdStockRecord.getId());
            if(entity != null && PdConstant.SUBMIT_STATE_2.equals(entity.getSubmitStatus())){
                return Result.error("库存初始化单已被提交，不能再次提交！");
            }
        }
        String recordId = pdStockRecordService.submit(pdStockRecord, pdStockRecord.getPdStockRecordDetailList(), PdConstant.RECODE_TYPE_3);
        Map<String,Object> result = new HashMap<>();
        result.put("recordId",recordId);
    ...
    public Result<?> audit(@RequestBody PdStockRecord pdStockRecord) {
        ...
        if(PdConstant.AUDIT_STATE_2.equals(entity.getAuditStatus()) || PdConstant.AUDIT_STATE_3.equals(entity.getAuditStatus())){
            return Result.error("库存初始化单已被审批，不能再次审批！");
        }
        Map<String, String> result = pdStockRecordService.audit(pdStockRecord, entity, PdConstant.RECODE_TYPE_3);
    ...
    public Result<?> cancel(@RequestBody PdStockRecord pdStockRecord) {
        ...
        if (PdConstant.SUBMIT_STATE_2.equals(entity.getSubmitStatus()) && PdConstant.AUDIT_STATE_1.equals(entity.getAuditStatus())) {
            ...
            return Result.error("当前库存初始化单状态已被审批或已撤回，不能撤回！");
        }
    }
    ...
    public Result<?> delete(@RequestParam(name = "id", required = true) String id) {
        ...
        if (PdConstant.SUBMIT_STATE_1.equals(entity.getSubmitStatus()) || PdConstant.SUBMIT_STATE_3.equals(entity.getSubmitStatus())) {
            ...
            return Result.error("当前库存初始化单状态非待提交或已撤回状态，不能删除！");
        }
    }
...
//        this.pdStockRecordService.delBatchMain(Arrays.asList(ids.split(",")));
     ...
     * @return
     */
    @GetMapping(value = "/queryPdProductStockInitDetailByMainId")
    public Result<?> queryPdProductStockInitDetailByMainId(@RequestParam(name = "id", required = true) String id) {
        PdStockRecordDetail pdStockRecordDetail = new PdStockRecordDetail();
        pdStockRecordDetail.setRecordId(id);
        List<PdStockRecordDetail> pdStockRecordDetailList = pdStockRecordDetailService.selectByMainId(pdStockRecordDetail);
    ...
    public ModelAndView exportXls(HttpServletRequest request, PdStockRecordDetail pdStockRecordDetail) {
        pdStockRecordDetail.setRecordType(PdConstant.RECODE_TYPE_3);
        ...
        mv.addObject(NormalExcelConstants.FILE_NAME, "库存初始化记录表");
        mv.addObject(NormalExcelConstants.CLASS, PdStockRecordInPage.class);
        mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("库存初始化记录表数据", "导出人:" + sysUser.getRealname(), "库存初始化记录表"));
        mv.addObject(NormalExcelConstants.DATA_LIST, exportList);
        return mv;
    }

    /**
     * 查询库存初始化明细  mcb  --20200224 用于统计查询
     *
     * @param pdStockRecordDetail
     * @param pageNo
     * @param pageSize
     ...
     * @return
     */
    @GetMapping(value = "/queryPdProductStockInitList")
    public Result<?> queryPdProductStockInitList(PdStockRecordDetail pdStockRecordDetail,
                                              @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                              @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
                                              HttpServletRequest req) {
     ...
     */
    @GetMapping(value = "pdProductStockInitQuery")
    public Result<?> pdProductStockInitQuery(PdStockRecord stockRecord,
                                            @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                            @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize) {
        stockRecord.setRecordType(PdConstant.RECODE_TYPE_3);
        stockRecord.setAuditStatus(PdConstant.AUDIT_STATE_2);//只查已通过的明细
        stockRecord.setInType(PdConstant.IN_TYPE_3);//只查调拨库存初始化的明细
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
    ...
    /**
     * 库存初始化统计报表  mcb  --20200616 用于统计查询  库存初始化统计报表
     */
    @GetMapping(value = "pdProductStockInitReportQuery")
    public Result<?> pdProductStockInitReportQuery(PdStockRecord stockRecord,
                                            @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                            @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize) {
        stockRecord.setRecordType(PdConstant.RECODE_TYPE_3);
        stockRecord.setAuditStatus(PdConstant.AUDIT_STATE_2);//只查已通过的明细
    ...
    /**
     * 库存初始化统计视图  mcb  --20200617 用于统计查询  库存初始化统计报表视图
     */
    ...
    public ModelAndView ReportQueryExportXls(HttpServletRequest request, PdStockRecord pdStockRecord) {
        ...
        // Step.4 AutoPoi 导出Excel
        ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
        mv.addObject(NormalExcelConstants.FILE_NAME, "库存初始化统计报表");
        mv.addObject(NormalExcelConstants.CLASS, PdStockRecordInExcle.class);
        mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("库存初始化统计报表数据", "导出人:" + sysUser.getRealname(), "库存初始化统计报表"));
        mv.addObject(NormalExcelConstants.DATA_LIST, exportList);
        return mv;
    }
}
